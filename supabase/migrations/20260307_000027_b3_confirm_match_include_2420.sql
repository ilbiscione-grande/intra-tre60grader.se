-- B3 hotfix: inkludera även 2420 (förskott) i konto-guard för bekräftad bankmatch.
...
begin
  ...
  -- Säkerställ att konton som register_invoice_payment kan använda finns och är aktiva.
  insert into public.chart_of_accounts (company_id, account_no, name, account_type, active)
  values
    (v_match.company_id, '1930', 'Företagskonto', 'asset', true),
    (v_match.company_id, '1510', 'Kundfordringar', 'asset', true),
    (v_match.company_id, '2420', 'Förskott från kunder', 'liability', true)
  on conflict (company_id, account_no)
  do update set
    active = true,
    name = excluded.name,
