-- A1: Invoice compliance fields and enriched invoice/customer snapshots.

alter table public.companies
  add column if not exists vat_no text,
  add column if not exists default_payment_terms_days integer not null default 30,
  add column if not exists late_payment_interest_rate numeric(5,2),
  add column if not exists invoice_terms_note text;
...
alter table public.invoices
  ...
  add column if not exists seller_vat_no text,
  add column if not exists buyer_reference text;

update public.invoices
set supply_date = coalesce(supply_date, issue_date),
    payment_terms_text = coalesce(payment_terms_text, '30 dagar netto');

do $$
begin
  if not exists (
    select 1 from pg_constraint
    where conname = 'invoices_payment_terms_text_nonempty'
      and conrelid = 'public.invoices'::regclass
  ) then
    alter table public.invoices
      add constraint invoices_payment_terms_text_nonempty
      check (coalesce(length(trim(payment_terms_text)), 0) > 0);
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_constraint
    where conname = 'invoices_supply_date_required'
      and conrelid = 'public.invoices'::regclass
  ) then
    alter table public.invoices
      add constraint invoices_supply_date_required
      check (supply_date is not null);
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_constraint
    where conname = 'companies_default_payment_terms_days_positive'
      and conrelid = 'public.companies'::regclass
  ) then
    alter table public.companies
      add constraint companies_default_payment_terms_days_positive
...
begin
  ...
  if v_vat_total > 0 and v_seller_vat_no is null then
    raise exception 'Company VAT number is required when invoice includes VAT.';
  end if;

  v_terms_days := greatest(coalesce(v_terms_days, 30), 0);
  v_terms_text := case
    when v_terms_days = 0 then 'Kontant'
    when v_terms_days = 1 then '1 dag netto'
    else format('%s dagar netto', v_terms_days)
  end;
  v_due_date := v_issue_date + v_terms_days;
  v_supply_date := v_issue_date;
